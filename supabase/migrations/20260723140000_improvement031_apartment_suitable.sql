-- IMPROVEMENT-031 — Filtro «Apto para piso» en la búsqueda pública.
--
-- El dato ya existe (`animals.apartment_suitable`) pero `animals_search` no lo
-- filtraba. Se añade el parámetro `p_apartment_suitable` (mismo patrón que los
-- filtros booleanos `p_good_with_*`).
--
-- La firma cambia (un parámetro más), así que hay que SOLTAR la función anterior
-- antes de recrearla: `create or replace` dejaría dos overloads y PostgREST no
-- sabría cuál llamar. Se recrea idéntica salvo el nuevo filtro. EXECUTE vuelve a
-- PUBLIC por defecto (como estaba: no había grants explícitos).
...
comment on function public.animals_search is
  'Listado público de animales con filtros combinables, búsqueda por texto (nombre/raza, IMPROVEMENT-021), filtro «apto para piso» (IMPROVEMENT-031), orden por distancia y paginación. La portada sale solo de fotos, nunca de vídeos (BUG-006). Security invoker: RLS aplica.';
