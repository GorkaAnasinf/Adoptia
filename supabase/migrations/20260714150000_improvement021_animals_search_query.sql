-- IMPROVEMENT-021: búsqueda por texto libre (nombre o raza) en el listado
-- público de animales.
--
-- Añade el parámetro `p_query` a `animals_search`. Como cambia la firma de la
-- función (un argumento más), NO basta `create or replace`: hay que soltar la
-- firma anterior de 14 argumentos para no dejar un overload ambiguo.
--
-- El patrón LIKE (`%término%`, con metacaracteres escapados) se monta en el
-- cliente (`searchToRpcArgs`) y llega ya listo en `p_query`; aquí solo se
-- aplica `ilike`. SECURITY INVOKER se mantiene: la RLS de animals/shelters
-- sigue filtrando dentro de la función.
--
-- Rendimiento: `ilike` secuencial es suficiente para el volumen actual (free
-- tier). Si el catálogo crece, procede `create extension pg_trgm` + índice GIN
-- sobre `lower(name)` y `lower(breed)`.
...
comment on function public.animals_search is
  'Listado público de animales con filtros combinables, búsqueda por texto (nombre/raza, IMPROVEMENT-021), orden por distancia y paginación. Security invoker: RLS aplica.';
