-- BUG-006 — `animals_search` servía la URL de un vídeo como imagen de portada.
--
-- FEATURE-020 filtraba `and m.type = 'photo'` en la subconsulta de `cover_url`
-- justo para esto. IMPROVEMENT-021 reescribió la función para añadir `p_query`
-- y perdió esa línea por el camino, así que un animal con vídeo y SIN foto de
-- portada devolvía `https://youtu.be/…` como `cover_url` — que acaba en el
-- `src` de un <Image> del listado: imagen rota.
--
-- Se restaura el filtro. Sin foto, `cover_url` es null y la tarjeta cae al
-- placeholder, que es el comportamiento correcto.
--
-- La firma no cambia respecto a IMPROVEMENT-021, así que `create or replace`
-- basta: no hay overload que soltar.
...
comment on function public.animals_search is
  'Listado público de animales con filtros combinables, búsqueda por texto (nombre/raza, IMPROVEMENT-021), orden por distancia y paginación. La portada sale solo de fotos, nunca de vídeos (BUG-006). Security invoker: RLS aplica.';
