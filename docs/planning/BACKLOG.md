...
- **Bloqueos:** ninguno. **Pendiente de despliegue:** ninguno — la migración de **FEATURE-062** (`20260725100000_feature062_events`: tablas `events`/`event_animals`/`event_attendees`, enum `event_status`, RPCs `events_upcoming`/`event_detail`, bucket `event-posters`) quedó **aplicada en producción el 2026-07-25** (`db push --include-all`; su timestamp era anterior al de BUG-010, ya aplicado). Ya en `main` y en producción con migraciones aplicadas: agenda completa (FEATURE-053→057), FEATURE-044, gestión de acogidas (FEATURE-058), historias felices Nivel 1 y 2 (FEATURE-035/059), filtro «Apto para piso» (IMPROVEMENT-031) y badge «Urgente» (FEATURE-060). **Nota gitflow:** las ramas se crean **desde `main`** (`develop` está desfasado); la tanda se libera directa a `main`.
