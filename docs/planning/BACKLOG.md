...
- **Siguiente:** **backlog sin items abiertos.** Toda la tanda de rediseño y las mejoras capturadas el 2026-07-23 están cerradas. **A cargo del usuario (no de la Manada):** borrado de datos de prueba (ver follow-ups) y verificación visual en dev local de las pantallas. Candidatos futuros anotados: Nivel 2.5 de historias (email de invitación tras adopción), caducidad del flag urgente (FEATURE-060), ampliar el buscador global (citas / nombre del adoptante).
- **Bloqueos:** ninguno. **Pendiente de despliegue:** ninguno — la migración de **FEATURE-060** (`20260723180000_feature060_urgent`: columna `animals.urgent` + recrea `animals_search` con `p_urgent` y orden urgentes-primero en recientes) quedó **aplicada en producción el 2026-07-24**. Ya en `main` y en producción con migraciones aplicadas: agenda completa (FEATURE-053+054+055+056+057, `555969b`), FEATURE-044, gestión de acogidas (FEATURE-058, `94868a5`), cards de enviadas (IMPROVEMENT-030, `f7adc7f`), historias felices Nivel 1 (FEATURE-035, `8d0a5e1`), filtro «Apto para piso» (IMPROVEMENT-031), historias felices Nivel 2 (FEATURE-059, `0e4dc54`) y badge «Urgente» (FEATURE-060). Solo código (sin migración) ya en prod: buscador global (FEATURE-061) y cabecera unificada de `/mi-cuenta` (IMPROVEMENT-032). **Nota gitflow:** `develop` está ~83 commits por detrás de `main` (la tanda se libera directa a `main`); las ramas se crean **desde `main`**, no desde `develop`.
...
- **Última actualización:** 2026-07-24 (**IMPROVEMENT-033** pulido de la cara pública —logo→home, Acogida/Necesidades al menú, pie rediseñado, home más compacta, Guías/legales al ancho del panel— y **BUG-009** reparado —test de vistas de la agenda roto por el rename de label y el rediseño a timeline—, ambos en `main` y prod. **Migración de FEATURE-060 aplicada en prod.** Suite 1196 verde. **Backlog vacío.** Antes, 2026-07-23: IMPROVEMENT-032, FEATURE-061, FEATURE-060, FEATURE-059, IMPROVEMENT-031, captura de 5 items, FEATURE-035 Nivel 1, IMPROVEMENT-030, FEATURE-058).
