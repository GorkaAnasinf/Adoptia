...
- **Progreso:** **FEATURE-056 hecha** (misma rama `feature/FEATURE-053-agenda-mensual`, circuito Manada completo) — **utilidades masivas de la agenda (F2b)**: botón **«Cerrar festivos»** (9 fijos nacionales + **Viernes Santo** calculado por cómputo de la Pascua en `src/lib/festivos.ts`) y **copiar día → pegar** en la selección (`estadoAOverride` en `src/lib/agenda.ts`). Todo por el mismo `upsert` de array de F2a; **sin migración**. QA: **suite 1142 verde**. Antes: **FEATURE-054** (F2a — pintar días + cerrar rangos, Decisión #49) y **FEATURE-053** (F1 — calendario mensual + `availability_overrides` + RPC reescrito; Decisiones #46-48; **con migración**). Faseado restante de la agenda: **F2c plantillas = FEATURE-057**, **F3 vistas anual/diaria = FEATURE-055** (ambos `recibido`). Antes: FEATURE-044 y la tanda previa (038–052) en producción.
- **Siguiente:** desplegar la agenda (**F1+F2a+F2b juntas**, misma rama; **incluye la migración de F1** → `supabase db push`) y arrancar **FEATURE-057** (F2c plantillas) o **FEATURE-055** (F3). **Verificación visual pendiente** de la agenda nueva y de FEATURE-040/041/042/043/044 (dev local autenticado).
- **Bloqueos:** ninguno. **Pendiente de despliegue:** **FEATURE-053 + FEATURE-054 + FEATURE-056** (misma rama; ⚠️ **con migración** `availability_overrides` de F1 — aplicar antes/junto al deploy) y FEATURE-044 (sin migraciones). **Nota gitflow:** `develop` está ~83 commits por detrás de `main` (la tanda se libera directa a `main`); esta rama se creó **desde `main`**, no desde `develop`.
...
- **Última actualización:** 2026-07-22 (FEATURE-056 — agenda F2b: festivos nacionales + copiar/pegar día).
