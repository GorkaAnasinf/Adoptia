...
- **Siguiente:** FEATURE-016 (registro de casas de acogida) — el último item del backlog.
- **En curso:** nada.
- **Bloqueos (operación manual pendiente):** ⚠️ `npx supabase db push` (7 migraciones pendientes: slug, citas, favoritos/alertas, moderación, perdidos/encontrados, apadrinamiento y estadísticas; bloqueado al agente por permisos) y ⚠️ secrets de GitHub Actions `SITE_URL` + `CRON_SECRET` (más `CRON_SECRET` en Vercel) para activar los crons.
...
- **Última actualización:** 2026-07-11 (cierre de FEATURE-015 — guías de adopción responsable).
