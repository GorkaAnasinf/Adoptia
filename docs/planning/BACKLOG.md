...
- **Bloqueos:** ninguno. **⚠️ Pendiente de despliegue:** **`supabase db push`** para aplicar la migración `20260714150000_improvement021_animals_search_query` (nuevo `p_query` en `animals_search`) — hasta entonces el buscador de texto no filtra en producción. Migraciones aplicadas hasta `20260714140000_bug004` (2026-07-14). **Pendiente de verificar** los tests de integración del RPC en un local reseteado (no se pudieron correr aquí por desincronía del entorno).
