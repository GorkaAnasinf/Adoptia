...
- **Siguiente:** **FEATURE-064 (Jornadas F3 — métricas → estadísticas e historias)**, ya planificada.
- **Bloqueos:** ninguno. **Pendiente de despliegue:** ninguno — la migración de **FEATURE-063** (`20260725160000_feature063_event_notifications`) quedó **aplicada en producción el 2026-07-25** y el cron `/api/cron/jornadas` está **programado** (GitHub Actions `jornadas.yml`, horario, con `SITE_URL`/`CRON_SECRET`). La de **FEATURE-062** también está en producción. **Jornadas F1+F2 completas y desplegadas.** Ya en `main` y en producción con migraciones aplicadas: agenda completa (FEATURE-053→057), FEATURE-044, gestión de acogidas (FEATURE-058), historias felices Nivel 1 y 2 (FEATURE-035/059), filtro «Apto para piso» (IMPROVEMENT-031) y badge «Urgente» (FEATURE-060). **Nota gitflow:** las ramas se crean **desde `main`** (`develop` está desfasado); la tanda se libera directa a `main`.
...
- **Última actualización:** 2026-07-25 (**BUG-011 cerrado — CI 100 % verde** tras arreglar `npm audit` (solo prod) y el E2E de onboarding + `test.slow()`. También hoy: **FEATURE-063 F2 (avisos)** cerrada (migración F2 pendiente de `db push` a prod + alta del cron), FEATURE-062 F1 en prod con capturas de manual. Queda F3 (FEATURE-064)).
