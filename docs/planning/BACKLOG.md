...
- **Siguiente:** cuatro items en `recibido` (2026-07-23, sin planificar): **FEATURE-059** (historias felices Nivel 2 — testimonios del adoptante, RGPD + moderación), **FEATURE-060** (badge «Urgente», requiere campo en BD), **FEATURE-061** (buscador global en la cabecera del área privada) e **IMPROVEMENT-032** (alinear las 6 subpáginas de `/mi-cuenta` con el dashboard). **A cargo del usuario (no de la Manada):** borrado de datos de prueba (ver follow-ups) y verificación visual en dev local de las pantallas rediseñadas.
- **Bloqueos:** ninguno. **Pendiente de despliegue:** **IMPROVEMENT-031** (filtro «Apto para piso») ⚠️ **una migración** → `supabase db push` aplica `20260723140000_improvement031_apartment_suitable` (suelta y recrea `animals_search` con `p_apartment_suitable`). Ya en `main` y en producción con migraciones aplicadas: agenda completa (FEATURE-053+054+055+056+057, `555969b`), FEATURE-044, gestión de acogidas (FEATURE-058, `94868a5`), cards de enviadas (IMPROVEMENT-030, `f7adc7f`) e historias felices Nivel 1 (FEATURE-035, `8d0a5e1`). **Nota gitflow:** `develop` está ~83 commits por detrás de `main` (la tanda se libera directa a `main`); las ramas se crean **desde `main`**, no desde `develop`.
...
- **Última actualización:** 2026-07-23 (IMPROVEMENT-031 hecha — filtro «Apto para piso» en `animals_search` + UI de filtros; ⚠️ una migración a aplicar. Antes hoy: captura de 5 items, FEATURE-035 Nivel 1, IMPROVEMENT-030 y FEATURE-058).
