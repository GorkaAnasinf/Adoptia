...
- **Siguiente:** tres items en `recibido` (2026-07-23, sin planificar): **FEATURE-060** (badge «Urgente», requiere campo en BD), **FEATURE-061** (buscador global en la cabecera del área privada) e **IMPROVEMENT-032** (alinear las 6 subpáginas de `/mi-cuenta` con el dashboard). **A cargo del usuario (no de la Manada):** borrado de datos de prueba (ver follow-ups) y verificación visual en dev local de las pantallas rediseñadas.
- **Bloqueos:** ninguno. **Pendiente de despliegue:** **FEATURE-059** (historias felices Nivel 2 — testimonios del adoptante) ⚠️ **una migración** → `supabase db push` aplica `20260723160000_feature059_adoption_stories` (tabla `adoption_stories` + RLS + bucket `story-media`). Ya en `main` y en producción con migraciones aplicadas: agenda completa (FEATURE-053+054+055+056+057, `555969b`), FEATURE-044, gestión de acogidas (FEATURE-058, `94868a5`), cards de enviadas (IMPROVEMENT-030, `f7adc7f`), historias felices Nivel 1 (FEATURE-035, `8d0a5e1`) y filtro «Apto para piso» (IMPROVEMENT-031, migración aplicada). **Nota gitflow:** `develop` está ~83 commits por detrás de `main` (la tanda se libera directa a `main`); las ramas se crean **desde `main`**, no desde `develop`.
...
- **Última actualización:** 2026-07-23 (FEATURE-059 hecha — historias felices Nivel 2: `adoption_stories` + moderación de la protectora dueña (Decisión #50), diálogo del adoptante en `/mi-cuenta`, `/panel/historias`, testimonios en la home con fallback al Nivel 1; ⚠️ una migración a aplicar. Suite 1193 verde. Antes hoy: IMPROVEMENT-031, captura de 5 items, FEATURE-035 Nivel 1, IMPROVEMENT-030, FEATURE-058).
