...
- **Siguiente:** FEATURE-013 (apadrinamiento y donaciones).
- **En curso:** nada.
- **Bloqueos (operación manual pendiente):** ⚠️ `npx supabase db push` (5 migraciones pendientes: slug, citas, favoritos/alertas, moderación y perdidos/encontrados; bloqueado al agente por permisos) y ⚠️ secrets de GitHub Actions `SITE_URL` + `CRON_SECRET` (más `CRON_SECRET` en Vercel) para activar los crons.
...
- **Última actualización:** 2026-07-11 (cierre de FEATURE-012 — perdidos y encontrados).
