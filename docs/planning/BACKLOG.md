...
- **Bloqueos:** ninguno. **Pendiente de despliegue:** **FEATURE-035** (Nivel 1 «Ya están en casa») ⚠️ **una migración** → `supabase db push` aplica el RPC `adopted_animals_recent`. Ya en `main` y en producción: agenda completa (FEATURE-053+054+055+056+057, `555969b`), FEATURE-044, gestión de acogidas (FEATURE-058, `94868a5`) y sus cards de enviadas (IMPROVEMENT-030, `f7adc7f`). **Nota gitflow:** `develop` está ~83 commits por detrás de `main` (la tanda se libera directa a `main`); las ramas se crean **desde `main`**, no desde `develop`.
