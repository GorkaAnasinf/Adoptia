...
- [ ] Protectora se loguea desde `/login` (sin redirect) → aterriza en `/panel` (el middleware ya la manda a `/panel/alta` si el alta no está enviada).
- [ ] Admin → `/admin`; adoptante → `/`.
- [ ] `?redirect=/mi-cuenta/favoritos` se respeta para cualquier rol.
- [ ] `?redirect=https://evil.com` y `?redirect=//evil.com` se ignoran → destino por rol.
- [ ] Login con Google (callback) aplica la misma lógica en ambos flujos (`code` y `token_hash`).
- [ ] Usuario sin fila en `profiles` o rol desconocido → `/` (sin crash).
- [ ] Tests del helper, LoginForm y callback en verde; lint y typecheck limpios.
